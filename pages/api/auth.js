    ...
    try {
        switch (method) {
            case 'POST':
                ...
                const { data } = await axios.post(`${process.env.ORIGIN}/user/login?_format=json&token=logout_token`, {
                    name: username,
                    pass: password,
                });
                if (!data.csrf_token) {
                    res.status(400).json({ status: 'error', error: 'Usuario o contraseñas incorrectos' });
                } else {
                    res.status(200).json({
                        success: true,
                        user: {
