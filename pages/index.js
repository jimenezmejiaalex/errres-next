...
function Home({ data }) {
  ...
  const caouselItems = data.slides.map(
    ({ id, title, description, buttonText, buttonUrl, imageInfo }) => ({
      id,
  ...
  const testimonialItems = data.testimonialsData.items.map((item) => ({
    ...
    image: `${process.env.NEXT_PUBLIC_SERVER_IMAGES}${item.te.media_image}`,
    ...
    <main className="flex flex-col">
      ...
      <div className="mx-8 md:mx-12 lg:mx-32 xl:mx-56">
        ...
        <section>
          <video
            controls
            src={`${process.env.NEXT_PUBLIC_SERVER_IMAGES}${data.media.media_video_file}`}
          ></video>
        </section>
        ...
        <section className="text-center py-4 flex flex-col justify-center items-center">
          ...
          <div
            className="text-leather py-4"
            dangerouslySetInnerHTML={{ __html: aboutMedata.body }}
          ></div>
          <img
            className="max-w-sm mt-10 mb-4"
            src={`${process.env.NEXT_PUBLIC_SERVER_IMAGES}${aboutMedata.image.media_image}`}
            alt="About me"
          />
          <h5 className=" text-base font-medium text-eden">
            {aboutMedata.name}
          </h5>
        </section>
      </div>
