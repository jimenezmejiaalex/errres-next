function PurchaseSummary({ products, envio, total }) {
  return (
    <div className="space-y-2 bg-white p-4 rounded shadow w-full">
      ...
      <div className=" overflow-hidden">
        <table className=" flex-col flex-no-wrap table-auto w-full leading-normal">
          ...
          <tbody className="flex-1 sm:flex-none">
            {products.map(({ title, price, id }, index) => (
              <tr
                key={`product-item-${index}-${id}`}
                className=" p-2 hover:bg-gray-100 table-row flex-col flex-no-wrap"
              >
                <td className="p-2">
